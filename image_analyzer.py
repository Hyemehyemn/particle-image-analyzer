...
class ImageAnalyzer:
    ...
    def particle_formula_variables(self, particle, length_scale=None):
        ...
        return {
            "A": particle["area"] * length_scale**2,
            "P": particle["perimeter"] * length_scale,
            "L": particle["major_axis"] * length_scale,
            "S": particle["minor_axis"] * length_scale,
            "C": particle["circularity"],
            "AR": particle["aspect_ratio"],
        }
    ...
    def draw_particle_labels(self, image, particles):
        ...
        for particle_id, particle in enumerate(particles, start=1):
            cv2.drawContours(image, [particle["contour"]], -1, (0, 255, 0), 2)
            center_x, center_y = (int(value) for value in particle["center"])
            cv2.putText(
                image, str(particle_id), (center_x, center_y),
